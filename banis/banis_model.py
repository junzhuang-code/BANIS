...
class BANIS():
    ...
    def generator_model(self, dim=8, depth=256):
        ...
        assert model.output_shape == (None, dim, dim, depth) # 注意：batch size 没有限制
    ...
    def discriminator_model(self, depth=64, drop_rate=0.3):
        ...
        model.add(LeakyReLU(alpha=0.2))
        model.add(Dropout(drop_rate))
        #model.add(Conv2D(4*depth, (5, 5), strides=(2, 2), padding='same', kernel_initializer='he_normal')) # 新增 ---
        #model.add(LeakyReLU(alpha=0.2))
        #model.add(Dropout(drop_rate)) # ---
    ...
    def train(self, A, B, EPOCHS=100, BATCH_SIZE=128, WARMUP_STEP=20, NUM_IMG=5):
        ...
        for epoch in range(EPOCHS):
            # ---Pretrain Stage ---
            # Select random batch of real instances
            idx = np.random.randint(0, A.shape[0], size=BATCH_SIZE)
            A_real = A[idx, :, :, :]
            ...
            with summary_writer.as_default():
                tf.summary.scalar('D_loss_A', Dloss_A[0], step=epoch)
                tf.summary.scalar('G_loss_A', Gloss_A[0], step=epoch)
                tf.summary.scalar('D_acc_A', Dloss_A[1], step=epoch)
                tf.summary.scalar('D_loss_B', Dloss_B[0], step=epoch)
                tf.summary.scalar('G_loss_B', Gloss_B[0], step=epoch)
                tf.summary.scalar('D_acc_B', Dloss_B[1], step=epoch)
                if mse != -1:
                    tf.summary.scalar('MSE_A', mse_A, step=epoch)
                    tf.summary.scalar('MSE_B', mse_B, step=epoch)
                    tf.summary.scalar('Identity_Loss', identity_loss, step=epoch)
                    tf.summary.scalar('Pair_Matched_Loss', pair_matched_loss, step=epoch)
                    tf.summary.scalar('MSE', mse, step=epoch)

            # Save the checkpoint at given interval
            if (epoch + 1) % int(interval) == 0:
            ...
            if (epoch + 1) % 50 == 0:
                # Input random noise and generate A_gen/B_gen.
            ...
            print("A: No.{0}: D_loss: {1}; D_acc: {2}; G_loss: {3}."\
                  .format(epoch, Dloss_A[0], Dloss_A[1], Gloss_A[0]))
            print("B: No.{0}: D_loss: {1}; D_acc: {2}; G_loss: {3}."\
                  .format(epoch, Dloss_B[0], Dloss_B[1], Gloss_B[0]))
            if mse != -1:
        ...
        # Save file
        np.save("./A_gen_banis.npy", A_gen_list)
        np.save("./B_gen_banis.npy", B_gen_list)
        np.save("./AB_rec_banis.npy", AB_rec_list)
        checkpoint.save(file_prefix = str(checkpoint_path))
